...
def load_data(filepath, pretrained_model_name_or_path, max_seq_len, batch_size):
    ...
    raw_val_data = pd.read_excel(io, sheet_name='ood')
...
if __name__ == "__main__":
    ...
    def build_argparser():
        p = argparse.ArgumentParser("Run evaluation with specified model checkpoint(s).")
        p.add_argument(
            "models",
            nargs="+",
            help="Path(s) to model checkpoint .bin, e.g. Lambdas_MaxVALAcc_0.9311.bin"
        )
        p.add_argument("--xlsx", default="processed_data1.xlsx", help="Path to processed .xlsx")
        p.add_argument("--bert", default="bert-base-chinese", help="BERT name or path")
        p.add_argument("--batch_size", type=int, default=256)
        p.add_argument("--max_seq_len", type=int, default=512)
        p.add_argument("--seed", type=int, default=3407)
        p.add_argument("--run", choices=["test", "ood", "both"], default="both")
        return p


    def load_model_or_die(model_path: str, device: torch.device) -> torch.nn.Module:
        ...
        if isinstance(obj, torch.nn.Module):
            ...
            raise TypeError(
                f"Loaded checkpoint is a dict (likely state_dict), not a full nn.Module: {model_path}\n"
                f"你需要先构建 model = ClassifyModel(...); 然后 model.load_state_dict(obj)."
            )
        else:
            raise TypeError(f"Unknown checkpoint type: {type(obj)} from {model_path}")
    ...
    train_iter, val_iter, test_iter, train_batch_count, val_batch_count, test_batch_count = load_data(
        args.xlsx, args.bert, args.max_seq_len, args.batch_size
    )
    ...
    for model_path in args.models:
        ...
        model = load_model_or_die(model_path, device)
        print("Loaded model type:", type(model))

        if args.run in ("test", "both"):
            metrics_test = Metrics(num_classes=2)
            t0 = time.time()
            result_test, test_acc, metrics_dict_test = evaluate_accuracy(
                test_iter, model, device, test_batch_count, metrics_test,
                figdump_cfg=figdump_cfg
            )
            print(f"[TEST] time: {time.time() - t0:.2f}s")
            print("[TEST] result_test:\n", result_test)
            print("[TEST] Metrics:", metrics_dict_test)

        if args.run in ("ood", "both"):
            metrics_val = Metrics(num_classes=2)
            t0 = time.time()
            result_ood, ood_acc, metrics_dict_ood = evaluate_accuracy(
                val_iter, model, device, val_batch_count, metrics_val,
                figdump_cfg=figdump_cfg
            )
            print(f"[OOD/VAL] time: {time.time() - t0:.2f}s")
            print("[OOD/VAL] result_ood:\n", result_ood)
            print("[OOD/VAL] Metrics:", metrics_dict_ood)
