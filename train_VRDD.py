...
def sliced_w2_squared(x: torch.Tensor, y: torch.Tensor, num_proj: int = 32) -> torch.Tensor:
    """Approximate W2^2 with Sliced Wasserstein (random 1D projections).
    x, y: [B, D] and must have same shape.
    """
...
class SimpleCSVLogger:
    """Append-only CSV logger with a fixed schema decided at first write."""
...
if __name__ == '__main__':
    ...
    batch_size, max_seq_len = 256, 512  # Damn loss limition
    Max_epoch = 150
    num_classes_news = 2
            ...
            if aux_losses is not None and ('_z_raw_norm' in aux_losses) and ('_instant_energy' in aux_losses):
                try:
                    with torch.no_grad():
                        ...
                        inst_energy = inst_energy.view(-1)

                        # ---- entropy from Dirichlet mean ----
                        p = alpha_det / (alpha_det.sum(dim=1, keepdim=True) + 1e-8)
                        entropy = -(p * torch.log(p + 1e-8)).sum(dim=1).view(-1)   # [B]

                        # ---- prototype distances -> margin ----
                        dist2 = model.dual_purifier.news_purifier.prototype_class_mindist2(z_raw_norm)  # [B,K]
                        d_sorted, _ = torch.sort(dist2, dim=1)
                        d1 = d_sorted[:, 0].view(-1)
                        d2 = d_sorted[:, 1].view(-1)
                        margin = (d2 - d1).clamp(min=0.0).view(-1)
                        ...
                        def _quantile(x, q):
                            ...
                            xs, _ = torch.sort(x)
                            k = int(max(0, min(xs.numel() - 1, math.floor(q * (xs.numel() - 1)))))
                            return xs[k]

                        # ---- scores ----
                        kappa = 0.5
                        gap = _zscore(d1) + kappa * _zscore(inst_energy)
                        hard = _zscore(entropy) + _zscore(1.0 / (margin + 1e-6))

                        # ---- mining hyperparams (mild by default) ----
                        q_shift = 0.95   # top 5% gap per class
                        q_hard = 0.80    # top 20% hard among non-shift per class
                        w_shift = 0.90
                        w_hard = 1.08
                        min_count = 16

                        num_classes = model.dual_purifier.news_purifier.num_classes

                        shift_mask = torch.zeros_like(batch_labels_1d, dtype=torch.bool, device=batch_labels_1d.device)
                        hard_mask  = torch.zeros_like(batch_labels_1d, dtype=torch.bool, device=batch_labels_1d.device)

                        # shift: per-class threshold, fallback to global
                        thr_shift_global = _quantile(gap, q_shift)
                        for k in range(num_classes):
                            idx = (batch_labels_1d == k)
                            if idx.sum().item() >= min_count:
                                thr_k = _quantile(gap[idx], q_shift)
                                shift_mask[idx] = gap[idx] > thr_k
                            else:
                                shift_mask[idx] = gap[idx] > thr_shift_global

                        # hard: among non-shift, per-class
                        for k in range(num_classes):
                            idx = (batch_labels_1d == k) & (~shift_mask)
                            if idx.sum().item() >= min_count:
                                thr_h = _quantile(hard[idx], q_hard)
                                hard_mask[idx] = hard[idx] > thr_h

                        sample_weights = torch.ones_like(entropy)  # [B]
                        sample_weights[shift_mask] = w_shift
                        sample_weights[hard_mask] = w_hard

                        # ---- debug: print once per epoch (first batch only) ----
                        if step == 0:
                            w_mean = float(sample_weights.mean().item())
                            w_min = float(sample_weights.min().item())
                            w_max = float(sample_weights.max().item())
                            shift_ratio = float(shift_mask.float().mean().item())
                            hard_ratio = float(hard_mask.float().mean().item())
                            print(
                                f"mining: w_mean={w_mean:.4f} (min={w_min:.3f}, max={w_max:.3f}), "
                                f"shift_ratio={shift_ratio:.3f}, hard_ratio={hard_ratio:.3f}"
                            )
                except Exception as e:
                    if not hasattr(model, "_mining_err_printed"):
                        print("[Mining] ERROR -> disable weighting:", repr(e))
